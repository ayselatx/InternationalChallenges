 ...
 *
 * @param pin: Digital pin number on the Arduino board to which the DHT11 sensor
 * is connected.
 */
 ...
 */
void DHT11::setDelay(unsigned long delay) { _delayMS = delay; }

/**
 * Reads raw data from the DHT11 sensor.
 * This method handles the direct communication with the DHT11 sensor and
 * retrieves the raw data. It's used internally by the readTemperature,
 * readHumidity, and readTemperatureHumidity methods.
 *
 * @param data: An array of bytes where the raw sensor data will be stored.
 *              The array must be at least 5 bytes long, as the DHT11 sensor
 * returns 5 bytes of data.
 ...
 * communication times out. Returns DHT11::ERROR_CHECKSUM if the data is read
 ...
 * Utilizes the readRawData method to retrieve raw data from the sensor and then
 * extracts the temperature from the data array.
 *
 * @return: Temperature value in Celsius. Returns DHT11::ERROR_TIMEOUT if
 * reading times out, or DHT11::ERROR_CHECKSUM if checksum validation fails.
 */
 ...
 * Utilizes the readRawData method to retrieve raw data from the sensor and then
 * extracts the humidity from the data array.
 *
 * @return: Humidity value in percentage. Returns DHT11::ERROR_TIMEOUT if
 * reading times out, or DHT11::ERROR_CHECKSUM if checksum validation fails.
 */
 ...
 * Utilizes the readRawData method to retrieve raw data from the sensor and then
 * extracts both temperature and humidity from the data array.
 *
 * @param temperature: Reference to a variable where the temperature value will
 * be stored.
 * @param humidity: Reference to a variable where the humidity value will be
 * stored.
 * @return: An integer representing the status of the read operation.
 *          Returns 0 if the reading is successful, DHT11::ERROR_TIMEOUT if a
 * timeout occurs, or DHT11::ERROR_CHECKSUM if a checksum error occurs.
 */
